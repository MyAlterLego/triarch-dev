...
describe('GET /api/platform/feature-requests — Phase 36 INCL filter (Pitfall 8)', () => {
  it('?inclusion_state=approved_for_build adds eq filter to where clause', async () => {
    const res = await GET(mkReq('http://localhost/api/platform/feature-requests?inclusion_state=approved_for_build'));
  ...
  it('?inclusion_state=garbage returns 400 invalid_inclusion_state', async () => {
    const res = await GET(mkReq('http://localhost/api/platform/feature-requests?inclusion_state=garbage'));
  ...
  it('GET (no inclusion_state) does NOT add inclusionState filter', async () => {
    const res = await GET(mkReq('http://localhost/api/platform/feature-requests'));
    expect(res.status).toBe(200);
